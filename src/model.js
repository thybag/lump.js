...
const Model = function(_data) {
  ...
  // Create watcher proxy to manage each object
  // provides access to get,set,on helpers + wraps access in proxies as needed to allow
        ...
        if (!parent._ignoreMagicMethods) {
          ...
          if (prop == 'on') {
            return function(event, callback) {
              return parent.on(`${event}:${context}`, callback);
            };
          }
        }
